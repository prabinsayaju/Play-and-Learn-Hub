...
export default function QuizGame() {
  ...
  const [question, setQuestion] = useState<QuizQuestion>(() => {
    const q = generateQuizQuestion(gradeNum, levelNum, usedQuestionsRef.current);
  ...
  // Timer gets faster as grade/level increases smoothly
  ...
  // Randomize options specifically for this question so it doesn't shuffle on every render
  ...
  const handleAnswer = (answer: string | null) => {
    ...
    if (answer === question.a) {
      ...
      if (nextProgress >= winTarget) {
        ...
        addStars(10);
        completeLevel('quiz', gradeNum, levelNum);
        setLevelComplete(true);
        confetti({
          particleCount: 150,
          spread: 100,
          origin: { y: 0.5 },
          colors: ['#04936D', '#FFD166', '#EF476F']
        });
      } else {
        soundManager.playCorrect();
        addStars(2);
        confetti({ particleCount: 50, spread: 50, origin: { y: 0.6 }, colors: ['#04936D', '#FFD166'] });
  ...
  return (
    <div className="flex flex-col items-center p-4 md:p-12 max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-3xl p-8 md:p-12 shadow-[0_8px_0_0_#CBD5E1] w-full max-w-2xl text-center mb-6 border-4 border-[var(--color-fun-green)]">
        <h2 className="text-xl md:text-2xl font-bold text-[var(--color-fun-purple)] mb-4 uppercase tracking-wider mt-2">Quiz Hero</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 w-full max-w-2xl">
        {currentOptions.map((opt, i) => (
          <motion.button
            ...
            className="bg-[var(--color-fun-purple)] shadow-[0_8px_0_0_#7B0E6C] text-white text-2xl md:text-3xl font-bold rounded-3xl py-6 active:translate-y-2 active:shadow-[0_0px_0_0_#7B0E6C] transition-colors relative overflow-hidden"
          >
            {opt}
      ...
      <AnimatePresence>
        ...
        {levelComplete && (
          ...
          >
            <div className="bg-white text-[var(--color-fun-navy)] text-3xl md:text-5xl font-bold px-8 md:px-12 py-8 md:py-12 rounded-[3rem] shadow-2xl flex flex-col items-center gap-6 border-8 border-[var(--color-fun-green)] pointer-events-auto text-center mx-4">
              Level {levelNum} Mastered! 
              <span className="text-[var(--color-fun-yellow)] drop-shadow-md">+10 Stars ⭐</span>
              <div className="flex gap-4 mt-4">
                <button 
                  onClick={() => navigate('/levels/quiz')}
                ...
                <button 
                  onClick={() => {
                    navigate(`/quiz/${gradeNum}/${levelNum + 1}`);
                    setProgress(0);
                    setLevelComplete(false);
                    setFeedback(null);
                  ...
                  className="bg-[var(--color-fun-purple)] text-white text-lg md:text-xl px-6 py-3 md:px-8 md:py-4 rounded-full shadow-[0_6px_0_0_#7B0E6C] active:translate-y-1 active:shadow-none transition-all"
                >
                  Next Level
                </button>
              </div>
